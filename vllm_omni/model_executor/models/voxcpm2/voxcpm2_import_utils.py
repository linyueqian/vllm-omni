...
def import_audio_vae_v2():
    ...
    AudioVAEV2, AudioVAEConfigV2 = _import_voxcpm2_attrs(
        "voxcpm.modules.audiovae", "AudioVAEV2", "AudioVAEConfigV2"
    )
    return AudioVAEV2, AudioVAEConfigV2


# ---------------------------------------------------------------------------
# Phase 2 (native AR) module imports
# ---------------------------------------------------------------------------


def import_minicpm4_model():
    """Import MiniCPMModel for residual_lm (manual KV cache)."""
    (MiniCPMModel,) = _import_voxcpm2_attrs(
        "voxcpm.modules.minicpm4.model", "MiniCPMModel"
    )
    return MiniCPMModel


def import_minicpm4_config():
    """Import MiniCPM4Config for constructing sub-model configs."""
    (MiniCPM4Config,) = _import_voxcpm2_attrs(
        "voxcpm.modules.minicpm4.config", "MiniCPM4Config"
    )
    return MiniCPM4Config


def import_scalar_quantization_layer():
    """Import ScalarQuantizationLayer (FSQ)."""
    (ScalarQuantizationLayer,) = _import_voxcpm2_attrs(
        "voxcpm.modules.layers", "ScalarQuantizationLayer"
    )
    return ScalarQuantizationLayer


def import_feat_encoder():
    """Import VoxCPMLocEnc (local encoder)."""
    (VoxCPMLocEnc,) = _import_voxcpm2_attrs(
        "voxcpm.modules.locenc", "VoxCPMLocEnc"
    )
    return VoxCPMLocEnc


def import_feat_decoder():
    """Import UnifiedCFM + VoxCPMLocDiTV2 (diffusion decoder)."""
    UnifiedCFM, VoxCPMLocDiTV2 = _import_voxcpm2_attrs(
        "voxcpm.modules.locdit", "UnifiedCFM", "VoxCPMLocDiTV2"
    )
    CfmConfig = _import_voxcpm2_attrs("voxcpm.modules.locdit", "CfmConfig")[0]
    return UnifiedCFM, VoxCPMLocDiTV2, CfmConfig


def import_voxcpm2_config_cls():
    """Import VoxCPMConfig pydantic model for building sub-configs."""
    (VoxCPMConfig,) = _import_voxcpm2_attrs(
        "voxcpm.model.voxcpm2", "VoxCPMConfig"
    )
    return VoxCPMConfig
...
def make_voxcpm2_model_for_omni(base_cls):
    ...
    class VoxCPM2ModelForOmni(base_cls):
        ...
        def _generate_with_prompt_cache(
            ...
            if not prompt_cache:
                # prompt_cache is None or empty dict
