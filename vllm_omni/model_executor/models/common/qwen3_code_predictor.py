...
class CodePredictorAttention(nn.Module):
    ...
    def __init__(self, config, *, prefix: str = "") -> None:
        ...
        # q/k/v.  Concatenating their weights along ``dim=0`` and feeding
        # the input through a single ``nn.Linear`` is *numerically
        # identical* to running them separately (same matmul, just packed),
        # but it cuts the per-step host-side overhead from three
        # ``cudaLaunchKernel`` calls down to one.  That alone is worth
        # ~0.8-1.5ms per ``talker_mtp`` call at the batch sizes we run in
        # production (5 layers x ~17 steps = 85 launches saved).
        #
        # We keep ``q_norm`` / ``k_norm`` (head-wise RMSNorm) and the
        # downstream attention math unchanged.  ``load_weights`` re-packs
        # the three separate HF tensors into ``qkv_proj.weight`` so HF
        # checkpoints continue to load as-is.
    ...
    def forward(
        ...
        # Single fused matmul, then split into q/k/v slices.  The split
        # is a no-op view on contiguous columns, so this is equivalent
        # to three separate Linears but with one host launch.
        qkv = self.qkv_proj(hidden_states)
        q_raw, k_raw, v_raw = qkv.split([self._q_size, self._kv_size, self._kv_size], dim=-1)
...
class CodePredictorMLP(nn.Module):
    ...
    def __init__(self, config, *, prefix: str = "") -> None:
        super().__init__()
        # Fused gate_up projection.  Same trick as ``qkv_proj`` above: the
        # HF reference uses two separate ``nn.Linear`` modules
        # (``gate_proj`` and ``up_proj``); we pack them into a single
        # ``nn.Linear`` whose output is split in half.  Numerically
        # identical, one host launch instead of two per layer per step.
