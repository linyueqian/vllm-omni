...
class VoxCPM2Config(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # -- Neutralise muP scaling for vllm --
        # vllm's MiniCPMModel/MiniCPMForCausalLM *always* applies:
        #   embed *= scale_emb
        #   residual += hidden * (scale_depth / sqrt(num_hidden_layers))
        #   output /= (hidden_size / dim_model_base)
        # VoxCPM2 was trained with use_mup=false, so we set these to no-ops.
